...
# parms:  a list of parameters for the model
  ...
  with(c(as.list(states), parms), { #extract parameters from 'parms' vector
    ...
    Psi_l = uniroot(leaf,c(-30,(b/K + Psi_s - h*rho*g)),c(as.list(states),parms))$root #Calculate leaf water potential
    ...
    dPsi_s = 0  #no change in soil water potential or 
   ...
   # dK = -K*(k.K/l.K)*((-X/l.K)^(k.K-1))*exp(-((-X/l.K)^k.K)) + theta*R #daily change in xylem conductance, mmol s-1 MPa-1 d-1 
    ...
    return(list(c(dPsi_s,dD,dS,dK),c(Psi_l=Psi_l,G=G,ABA=ABA,X=X, P=P, R=R, B=B, M=M)))   #return a list of of both derivatives and other state variables
  })
}
